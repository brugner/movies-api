...
public class RatingService : IRatingService
{
    ...
    public async Task<bool> DeleteAsync(Guid movieId, Guid userId, CancellationToken cancellationToken = default)
    {
        return await _ratingRepository.DeleteAsync(movieId, userId, cancellationToken);
    }
    ...
    public async Task<bool> RateAsync(Guid movieId, int rating, Guid userId, CancellationToken cancellationToken = default)
    {
        if (rating is <= 0 or > 5)
        {
        ...
        return await _ratingRepository.RateAsync(movieId, rating, userId, cancellationToken);
    }
}